<?php
        ...
        try {
            ...
            if (empty($location) || empty($surfaceArea) || empty($seedingDate) || empty($waterDepts) || empty($breadingMethod)) {
                ...
                if ($user_id != null) {
                    ...
                    $countFarm_query = $connection->prepare("SELECT COUNT(*) AS total FROM oysterFarm WHERE user_ID = ?");
    ...
    if (isset($_POST["deleteFarm"])) {
        try {
            ...
            if (!$farmID) {
                ...
                $q = $connection->prepare("DELETE FROM oysterFarm WHERE farm_ID = ?");
                $q->bind_param("i", $farmID);

                if ($q->execute())
                    // echo "<script>window.alert('Farm deleted successfully.'); window.location.reload();</script>";
    ...
    try {
        ...
        while ($farm_row = $farms_tb->fetch_assoc())  $farms[] = $farm_row;

        /**
         harvest history
         */
        if ($user_id !== null)  $harvest_q = "SELECT * FROM harvestHistory WHERE user_ID = " . $user_id;
        else  $harvest_q = "SELECT * FROM harvestHistory";
        ...
        /**
         weather
         */
        if ($user_id !== null)  $weather_q = "SELECT * FROM weatherHistory WHERE user_ID = " . $user_id;
        else  $weather_q = "SELECT * FROM weatherHistory";
    ...
    foreach ($harvest as $row) {
        ...
        // Try to use the real farm name from $farms
        foreach ($farms as $f) {
    ...
    <body>
        ...
        >
            ...
            <div class="d-flex align-items-center">
                ...
                >
                    ...
                    <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger" style="width: 5px; height: 10px; font-size: 11px;">
                ...
                <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                    <section>
                        <div class="container text-center" style="margin-bottom: 30px;" id="weather-updates">
                            ...
                            <div class="card h-100" style="border:none; border-radius:16px; overflow:hidden; box-shadow:0 4px 20px rgba(0,180,220,0.15);">
                                    ...
                                    >
                                        ...
                                        <div class="d-flex border-top">
                                            <div class="flex-fill text-center py-2 px-3" style="border-right: 1px solid #dee2e6;">
                                                ...
                                                <span class="fw-semibold small farm-tide-type" id="tide-type" style="color:#023e8a;">—</span>
                                            </div>
                                            <div class="flex-fill text-center py-2 px-3">
                                                ...
                                                <span class="fw-semibold small farm-water-temp" id="water-temp" style="color:#023e8a;">—</span>
                                            </div>
                                        </div>
                                    ...
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <form method="post" action="farm.dashboard.rtl.php" class="d-inline">
                                ...
                                <div class="modal fade" id="harvestFarm-modal-<?= $farm['farm_ID'] ?>" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content">
                                            ...
                                            <?php
                                                $hw_windows = [
                                                    'Triploids'                => ['min'=>8,  'max'=>18],
                                                    'Tetraploids (4n)'         => ['min'=>10, 'max'=>20],
                                                    'cross'                    => ['min'=>12, 'max'=>20],
                                                    'line'                     => ['min'=>14, 'max'=>24],
                                                    'family'                   => ['min'=>14, 'max'=>24],
                                                    'Fertilization'            => ['min'=>12, 'max'=>24],
                                                    'Temperature Manipulation' => ['min'=>10, 'max'=>18],
                                                    'Environmental Cues'       => ['min'=>12, 'max'=>20],
                                                    'Stake Method'             => ['min'=>15, 'max'=>24],
                                                    'Cultch / Substrate'       => ['min'=>14, 'max'=>24],
                                                    'Seed Collection'          => ['min'=>18, 'max'=>36],
                                                ];
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <!-- Harvest form -->
                                                <form method="post" action="farm.dashboard.rtl.php">
                                                    <input type="hidden" name="farmID" value="<?= $farm['farm_ID'] ?>">
                                ...
                                <div class="modal fade" id="reseedFarm-modal-<?= $farm['farm_ID'] ?>" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content">
                                            ...
                                            <form method="post" action="farm.dashboard.rtl.php" class="modal-body">
                                                <input type="hidden" name="farmID" value="<?= $farm['farm_ID'] ?>">
                                ...
                                <div class="modal fade" id="editFarm-modal-<?= $farm['farm_ID'] ?>" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="editFarmLabel-<?= $farm['farm_ID'] ?>" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <form method="post" action="farm.dashboard.rtl.php" enctype="multipart/form-data" class="modal-body">
                ...
                <div class="modal fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        ...
                        <form method="post" action="farm.dashboard.rtl.php" enctype="multipart/form-data" class="modal-body">
                            ...
                            <div class="form-floating">
                                <input type="text" step="0.01" min="0" name="surfaceArea" class="form-control" id="floatingInput" placeholder="Surface Area" require>
                                <label for="floatingInput">Surface Area (square meter)</label>
                            </div>
                            <div class="form-floating">
                                <input type="date" name="seedingDate" class="form-control" id="floatingInput" placeholder="Seeding Date" require>
                                <label for="floatingInput">Seeding Date"</label>
                            </div>
                            <div class="form-floating">
                                <input type="number" step="0.01" min="0" name="waterDepts" class="form-control" id="floatingInput" placeholder="Water Depts" require>
                                <label for="floatingInput">Water Depts (meters)</label>
                            </div>
                            <!-- <div class="form-floating"> -->
                            ...
                            <div class="breed-type" id="breed-type">
                                ...
                                <div class="dropdown">
                                    <button class="btn btn-outline-secondary dropdown-toggle text-start" type="button" id="breedDropdown" data-bs-toggle="dropdown" aria-expanded="false">
                                        Natural Breeding
                                    </button>
                                    <div class="dropdown-menu p-3 dropdown-radio w-100">
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="breedtype" id="lineBreedingModal" value="Stake Method">
                                            <label class="form-check-label" for="lineBreedingModal">Stake Method (Tulos / Patusok)</label>
                                        </div>
                                    </div>
                                </div>
                                <div class="dropdown">
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu p-3 dropdown-radio w-100">
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="breedtype" id="lineBreedingModal" value="Temperature Manipulation">
                                            <label class="form-check-label" for="lineBreedingModal">Temperature Manipulation</label>
                                        </div>
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="breedtype" id="familyBreedingModal" value="Environmental Cues">
                                            <label class="form-check-label" for="familyBreedingModal">Environmental Cues</label>
                                        </div>
                                        <div class="form-check mb-2">
                                            <input class="form-check-input" type="radio" name="breedtype" id="crossBreedingModal" value="Fertilization">
                                            <label class="form-check-label" for="crossBreedingModal">Fertilization</label>
                                        </div>
                                    </div>
                                </div>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu p-3 dropdown-radio w-100">
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="breedtype" id="lineBreedingModal" value="Triploids">
                                            <label class="form-check-label" for="lineBreedingModal">Triploids</label>
                                        </div>
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="breedtype" id="familyBreedingModal" value="Tetraploids (4n)">
                                            <label class="form-check-label" for="familyBreedingModal">Tetraploids (4n)</label>
                                        </div>
                                    </div>
                                </div>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu p-3 dropdown-radio w-100">
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="breedtype" id="lineBreedingModal" value="Cultch / Substrate">
                                            <label class="form-check-label" for="lineBreedingModal">Cultch / Substrate</label>
                                        </div>
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="breedtype" id="familyBreedingModal" value="Seed Collection">
                                            <label class="form-check-label" for="familyBreedingModal">Seed Collection</label>
                                        </div>
                                    </div>
                                </div>
        ...
        <!-- <script>
            ...
            function renderCalendar(date) {
                ...
                // Number of days in the previous month (used for leading disabled buttons)
                ...
                for (let day = 1; day <= totalDays; day++) {
                    ...
                    // Fill the remaining cells with next month's leading days (disabled)
                    const cellsSoFar = firstDay + totalDays;
                    const totalCells = 7 * 6; // always show 6 weeks for consistent layout
                    const nextDays = totalCells - cellsSoFar;
        ...
        <div class="modal fade" id="editProfileModal" tabindex="-1" aria-labelledby="editProfileModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg modal-dialog-scrollable">
                <div class="modal-content">
                    ...
                    <form method="POST" action="farm.dashboard.rtl.php" enctype="multipart/form-data">
                        <div class="modal-body">
        ...
        <div class="modal fade" id="reportModal" tabindex="-1" aria-labelledby="reportModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg modal-dialog-scrollable">
                <div class="modal-content">
                    ...
                    <form method="POST" action="farm.dashboard.rtl.php" enctype="multipart/form-data" id="reportForm">
                        ...
                        <ul class="nav nav-pills mb-4 gap-2" id="reportTypeTabs" role="tablist">
                            ...
                            <li class="nav-item" role="presentation">
                                <button class="nav-link d-flex align-items-center gap-1" id="tab-request" data-bs-toggle="pill" data-bs-target="#report-request" type="button" role="tab">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" class="bi bi